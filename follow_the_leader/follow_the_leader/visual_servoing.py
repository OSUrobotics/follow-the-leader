...
from follow_the_leader_msgs.msg import Point2D, TrackedPointGroup, TrackedPointRequest, Tracked3DPointGroup, Tracked3DPointResponse, VisualServoingRequest, States, StateTransition
...
class VisualServoingNode(TFNode):
    def __init__(self):
        ...
        self.point_response_sub = self.create_subscription(Tracked3DPointResponse, 'point_tracking_response',
                                                           self.handle_3d_point_tracking_response, 1)
        self.joint_state_sub = self.create_subscription(JointState, 'joint_states', self.handle_joint_state, 1, callback_group=self.cb_group)
    ...
    def handle_joint_state(self, msg: JointState):
        ...
        cur_pos = self.lookup_transform(self.base_frame.value, self.tool_frame.value, rclpy.time.Time(), sync=False, as_matrix=True)[:3,3]
        if self.last_tool_pos is None or np.linalg.norm(cur_pos - self.last_tool_pos) > self.servo_joint_state_dist.value:
