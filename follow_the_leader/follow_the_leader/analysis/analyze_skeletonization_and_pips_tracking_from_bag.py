...
class BagReader:
    ...
    def query_closest(self, topic_name, ts):
        ...
        rows = self.cursor.execute("SELECT timestamp, data FROM messages WHERE topic_id = {} ORDER BY ABS(timestamp - {}) ASC LIMIT 1".format(topic_id, ts))
        for ts, data in rows:
            yield ts, deserialize_message(data, topic_msg_type)
...
def show_skel_results(bag_file):
    ...
    for _, msg in reader.query('/image_mask'):
        ...
        if len(sbs):
            ...
            chosen_skel_inflated = Image.fromarray(np.dstack([zeros, chosen_skel_inflated * 255, zeros]).astype(np.uint8))
            ...
            for px in node_px:
                ...
                ul = px - (r,r)
                br = px + (r,r)
                draw.ellipse((*ul, *br), fill='red', outline='black',
                             width=1)
...
def show_pips_tracking(bag_file):
    ...
    for raw_ts, msg in all_imgs:
        ...
        if img_queue.is_full and ts >= queue[current_target]['ts']:
            ...
            last_curve_pts = np.array([[pt.x, pt.y, pt.z] for pt, pid in zip(closest_curve.points, closest_curve.ids) if pid == 0])
