    ...
    for pt_list in pt_lists:
        ...
        for i, branch_pts in enumerate(pt_list):
            for pt_1, pt_2 in zip(branch_pts[:-1], branch_pts[1:]):
                ...
                cyl = pv.Cylinder(cyl_center, direction=pt_2 - pt_1, radius=rad, height=np.linalg.norm(pt_2 - pt_1),
...
def process_bag_file(file, video_output=None, gif_output=None, leader_rad=0.01, sb_rad=0.01):
    ...
    model_renders = models_to_renders(
        backfill_source_against_timestamps(reader, all_ts, topic='/tree_model', use_raw_ts=False), pose_interp,
        main_rad=leader_rad, side_rad=sb_rad)
    diagnostics = [bridge.imgmsg_to_cv2(msg, desired_encoding='rgb8') for msg in backfill_source_against_timestamps(reader, all_ts, '/model_diagnostic', use_raw_ts=True)]
    masks = [bridge.imgmsg_to_cv2(msg, desired_encoding='mono8') for msg in backfill_source_against_timestamps(reader, all_ts, topic='/image_mask', use_raw_ts=False)]
    ...
    if video_output is not None:
        ...
        for rgb, diag, mask in zip(all_images, diagnostics, masks):
            ...
            if writer is None:
                writer = cv2.VideoWriter(video_output, format, fps_float,
        ...
        for img in model_renders:
            if writer is None:
                writer = cv2.VideoWriter(video_output.replace('.mp4', '_render.mp4'), format, fps_float, (img.shape[1], img.shape[0]))
    ...
    for folder_id in to_proc:
        ...
        for run in os.listdir(subfolder):
            ...
            process_bag_file(bag_file,
                             ...
                             gif_output=os.path.join(root, 'videos', 'bag_gifs', '{}_{}.gif'.format(folder_id, run)), leader_rad=leader_radius, sb_rad=sb_radius)
