...
class BSplineFit(BSplineCurve):
    ...
    def iteratively_fit(self, points: list[np.ndarray]):
        ...
        if event.button == 1:  # projection on convex hull LEFT
            ...
            if len(self.points_to_fit) > self.degree:
                if not self.is_initialized:
                    new_control_points, points_in_t, residuals, rank = self.simple_fit(
                        self.points_to_fit
                    )
                else:
                    new_control_points, points_in_t, residuals, rank = self.renorm_fit(
                ...
                if residuals.size == 0 or (residuals < 1e-3).all():
                    ...
                    self.parameterize_chord(
                        self.data_pts, True
                    )  # TODO: i don't think this should work beyond first point
                    new_control_points, points_in_t, residuals, rank = (
                        self.extend_curve([self.points_to_fit[-1]])
                    )
