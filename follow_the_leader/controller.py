...
class FollowTheLeaderController_ROS(Node):
    ...
    def __init__(self):
        ...
        self.srv_start = self.create_service(Trigger, 'servo_start', self.start, callback_group=self.service_handler_group)
        ...
        self.mask_sub = self.create_subscription(Image, '/image_mask', self.process_mask, 1, callback_group=self.img_subscriber_group)
    ...
    def process_mask(self, msg):
        ...
        if self.publish_diagnostic:
            ...
            diag = cv2.arrowedLine(diag, cint, (center_pt + arrow_base_len * self.last_action / np.linalg.norm(self.last_action)).astype(int), (255, 255, 0), 8)
